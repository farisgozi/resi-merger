...
async function initRedis() {
    if (!process.env.REDIS_URL) {
        console.log('📁 REDIS_URL not set - cannot use Redis auth');
        return null;
    }
    
    try {
        console.log('🔗 Connecting to Redis for auth state...');
        ...
        console.log('✅ Redis connected for auth state:', pingResult);
...
async function makeAuthState() {
    ...
    if (redisClient) {
        console.log('🔑 Using Redis auth state with baileys-redis-auth');
        
        try {
            // Use baileys-redis-auth library
            const authState = await useRedisAuthState(redisClient, 'wa-session');
    ...
    // If Redis fails, stop the application - no file fallback
...
class WhatsAppPDFBot {
    ...
    async clearAuthState() {
        try {
            ...
            if (redisClient) {
                try {
                    // Clear Redis session using pattern
                    const keys = await redisClient.keys('wa-session:*');
                    if (keys.length > 0) {
                        await redisClient.del(...keys);
                        console.log(`🔗 Cleared ${keys.length} Redis session keys`);
                    } else {
                        console.log('🔗 No Redis session keys found to clear');
                    }
                } catch (error) {
                    console.error('❌ Error clearing Redis session:', error.message);
                }
            }
